...
: panicked(false), asleep(false), debug(false)
{
	...
	screen = SDL_SetVideoMode(160, 144, 8, SDL_HWSURFACE | SDL_DOUBLEBUF);
	if(screen == NULL)
	{
		panic();
		return;
	...
	PALETTE[1] = SDL_MapRGB(screen->format, 0x80, 0x80, 0x80);
	PALETTE[2] = SDL_MapRGB(screen->format, 0x60, 0x60, 0x60);
	PALETTE[3] = SDL_MapRGB(screen->format, 0x20, 0x20, 0x20);
	PALETTE[4] = SDL_MapRGB(screen->format, 0x00, 0x00, 0x00);//disabled
	...
	if(*LCDC & 0x01)
	{
		for(int screen_y = 0; screen_y < 144; ++screen_y)
		{
			for(int screen_x = 0; screen_x < 160; ++screen_x)
			{
				...
				putpixel(screen, screen_x % 160, screen_y % 144, bg_pal[data]);
			}
		}
	}
		...
		for(int j = 0; j < 1; ++j)
		{
			...
			for(int y = 0; y < 16; ++y)
			{
				for(int x = 0; x < 8; ++x)
				{
					...
					putpixel(screen, spriteset[i].x_pos, 
...
//	static int i = 0;
	...
	/*	if(i++ < FRAMESKIP)
		...
		else
		{
			...
			{
				asleep = true;
				SDL_FillRect(screen, NULL, PALETTE[4]);
				SDL_Flip(screen);
			}
		}
	}
